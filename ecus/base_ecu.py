...
class BaseECU(ABC):
    ...
    def get_write_regions(self) -> Dict[str, Tuple[int, int, str]]:
        """
        Returns named flash regions for writing.

        Defaults to get_flash_regions(). Override when write addresses differ
        from read addresses (e.g. bootloader is read-only).
        """
